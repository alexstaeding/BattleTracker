...
public class TrackerListener {
    ...
    public void onQuit(ClientConnectionEvent.Disconnect event) {
        ...
        for (MemberRepository memberRepository : plugin.getTrackerManager().getInterfaces().values()) {
            if (!memberRepository.getRecapManager().getDeathRecaps().containsKey(event.getTargetEntity().getName()))
                continue;

            memberRepository.getRecapManager().getDeathRecaps().remove(event.getTargetEntity().getName());
        }
    }
