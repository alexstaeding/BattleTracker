...
public class TrackerExecutor extends CustomCommandExecutor {
    ...
    public void topCommand(CommandSender sender, int amount) {
        ...
        Map<Member, Float> sortedRecords = TrackerUtil.getSortedRecords(tracker, amount);

        int i = 1;
        for (Map.Entry<Member, Float> recordEntry : sortedRecords.entrySet()) {
    ...
    public void rankCommand(CommandSender sender, OfflinePlayer player) {
        MessageManager messageManager = plugin.getMessageManager();
        Optional<Member> opRecord = tracker.getRecord(player);
        if (!opRecord.isPresent()) {
            sender.sendMessage(messageManager.getFormattedMessage(player, "recordNotFound"));
            return;
        }

        Member member = opRecord.get();
        DecimalFormat format = new DecimalFormat("0.##");
        String message = messageManager.getFormattedMessage(player, "rankingText");
        message = message.replace("%kd_ratio%", format.format(member.getStat(StatTypes.KD_RATIO)));

        for (StatType type : StatTypes.values()) {
            message = message.replace("%" + type.getInternalName() + "%", format.format(member.getStat(type)));
        }
    ...
    public void setCommand(CommandSender sender, OfflinePlayer player, String statType, float value) {
        ...
        Member member = opRecord.get();
        if (!member.getStats().containsKey(statType.toLowerCase())) {
    ...
    public void versusCommand(CommandSender sender, OfflinePlayer player1, OfflinePlayer player2) {
        ...
        DecimalFormat format = new DecimalFormat("0.##");
        Member member1 = tracker.getOrCreateRecord(player1);
        Member member2 = tracker.getOrCreateRecord(player2);
        ...
        for (Map.Entry<String, Float> statEntry : member1.getStats().entrySet()) {
            versusMessage = versusMessage.replace("%" + statEntry.getKey() + "_1%", format.format(statEntry.getValue()));
        }

        for (Map.Entry<String, Float> statEntry : member2.getStats().entrySet()) {
