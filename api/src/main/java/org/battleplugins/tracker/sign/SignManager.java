...
public interface SignManager {
     ...
     */
    Map<Location, LeaderboardSign> signs;
     ...
     */
    default void refreshSignContent(Sign sign) {
        ...
        Optional<MemberRepository> opTracker = plugin.getTrackerManager().getInterface(trackerName);
        if (!opTracker.isPresent())  {
            Log.debug("A tracker could not be found for " + trackerName + "!");
            return;
        }

        Map<Member, Float> recordsMap = TrackerUtil.getSortedRecords(opTracker.get(), extraLines + normalSignLines);
        if (recordsMap.isEmpty())
            return;

        List<Member> members = new ArrayList<>(recordsMap.keySet());
        int recordIndex = 0;
        for (int i = 0; i < lines.length; i++) {
            String line = lines[i];
            if (members.size() <= recordIndex) {
            ...
            formattedLine = formattedLine.replace("%player_name%", members.get(recordIndex).getName());
            formattedLine = formattedLine.replace("%value%", String.valueOf(members.get(recordIndex).getStat(statType)));

            TrackerUtil.replaceRecordValues(formattedLine, members.get(recordIndex));
        ...
        if (!updateSignsBelow || signsBelow.isEmpty() || members.size() <= recordIndex)
            return;
